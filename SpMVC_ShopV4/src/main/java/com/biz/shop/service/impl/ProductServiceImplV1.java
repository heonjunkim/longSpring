...
public class ProductServiceImplV1 implements ProductService {
	...
	public int update(ProductVO vo) {
		...
		// Update를 수행하기 전에 삭제Flag 칼럼을 강제로 null로 설정하여
		// 해당 상품이 삭제표시가 되지 않도록 만든다.
	...
	/*
	 * 상품정보, 거래처정보, 회원정보 와
	 * 	 같은 데이터를 통상 Master Data 라고 한다. 
	 * Master Data는
	 * Work Data(매입매출, 거래처외상정보, 회원로그인 로그와 같은)의
	 * 기준이되는 값을 가지는 Data Table들이다.
	 * 
	 * 어플을 사용해서 어떤 업무를 진행하는 과정에서
	 * WorkData에 저장되는 데이터는 
	 * 		필수적으로 Master Data와 join관계가 맺어진다.
	 * 만약 WorkData와 Master Data를 Join하여 어떤 통계정보를 찾고자할때
	 * Master에 해당하는 데이터(코드와 일치하는)가 없으면
	 * WorkData의 정보가 어떤내용인지 확인이 어려워 지는경우가 많다.
	 * 
	 * 통상적으로
	 * Master Data는 원칙상 PK 칼럼값은 변경하지 않고
