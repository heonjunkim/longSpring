...
public class ProductVO {
	...
	// DB 연동을 할때 CHAR(1) TinyInt 형으로 선언된 칼럼과 연동하기 위해
	// 사용하는 type이 type 0 ~ 255, -128 ~ 127까지의 값만을 저장하는 데이터
	// 메모리 공간 크기가 1byte(8bit)
	// int 형은 java에서 4byte 크기를 사용
	// String 형은 wrapper 클래스이다 보니 실제적으로 상당히 큰
	// 메모리 공간을 차지한다.
	// 단순히 Flag와 같은 데이터를 취급할때는 char, byte 형으로 사용하면
	// 메모리를 절약할 수 있다.
	 ...
	 * 
	 * 때문에 
	 * 이 칼럼에 타입을 byte형이 아닌 Byte wrapper 클래스형으로 설정한다.
	 * 
	 */
