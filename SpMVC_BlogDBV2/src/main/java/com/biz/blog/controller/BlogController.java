...
public class BlogController {
	...
	// htttp://localhost:8080/blog/blog/list 주소로 req했을때
	// 응답할 함수
	// method=RequestMethod.GET : req를 할때  <a href=주소> 를 클릭했을때
	// 응답하는 method
...
//		System.out.println(blogList.get(0).getBl_title());
	...
	public String input(Model model) {
		...
		// 공백의 새로운 BlogVO 만들어서 write.jsp로 보내준다.
		model.addAttribute("BLOG",new BlogVO());
		return "write";
	}

	/*
	 * @ModelAttribute form에서 input에 입력한 문자열이 전송되어 오면 input tag의 변수(name)을 분석하여 VO
...
//		model.addAttribute("USER", blogVO.getUser());
	...
	public String update(String seq, Model model) {
		...
		// update 할 데이터를 SELECT 해 오기
		BlogVO blogVO = bService.findBySeq(long_seq);
		
		// update 할 데이터를 model에 싣기
		model.addAttribute("BLOG", blogVO);
	...
	public String update(BlogVO blogVO, Model model) {
		...
		// 	+blogVO.getBL_seq(); 이런 코드를 자동으로 만들어주더라
