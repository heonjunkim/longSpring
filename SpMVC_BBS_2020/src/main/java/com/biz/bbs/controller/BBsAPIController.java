...
public class BBsAPIController {
	...
	// http:127.0.0.1:5500 에서 요청이 api 요청이 오면, CORS Policy를 무시하고
	// 응답하라
//	@CrossOrigin("http://127.0.0.1:5500")
	...
	// form에 데이터를 입력하고 submit을 수행하면 데이터를 수신하여 insert 하라
	// VO 클래스를 매개변수로 지정하고 데이터를 수신한다.
	// @ModelAttribute를 사용하면 form에서 submit한 데이터를 VO에 담기
	...
	// form에 데이터를 입력하고 submit을 수행하면 데이터를 update 하라
	...
	public String bbs_delete(@RequestBody Map<String, String> data) {
		log.debug("DELETE method Type으로 요청된 method");
		log.debug("시퀀스 값 {}", data.get("seq").toString());
		return "bbs_delete";
	}
