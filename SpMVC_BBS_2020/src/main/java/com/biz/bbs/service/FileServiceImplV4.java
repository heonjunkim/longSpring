 ...
 * 
 * 특히, 프로젝트 구조를 알게 되는 경우 같은 이름의 파일을 엉뚱한 파일로 
 * 업로드하여 원래 저장되어 있던 파일을 변형하는 일들이 발생할 수 있다.
 * 
...
public class FileServiceImplV4 extends FileServiceImplV1 {
	...
	public String fileUp(MultipartFile file) {
		...
		// mk dir : 운영체제에서 파일 만들때 쓰는 명령어
		if(!dir.exists()) {
			// mkdir()은 제일 끝의 폴더 1개 생성
			// mkdirs()은 모든 경로의 폴더를 한꺼번에 생성
			dir.mkdirs();
		}
		// 원본 파일이름이 된다.
		 ...
		 * 3. UUID 값이 부착된 파일이름은 서버에 업로드가 될것이고
		 * 만약 해커가 해당 파일이름을 알게되어 동일한 이름의 파일을 만들어서
		...
		// UUID가 부착된 파일이름을 Controller로 return 하여 DB에 저장하는 
		// 용도로 사용한다.
		return saveFileName;
	}
	/*
