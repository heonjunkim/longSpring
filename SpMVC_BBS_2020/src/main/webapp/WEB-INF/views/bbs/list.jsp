...
<script>
	document.addEventListener("DOMContentLoaded", function() {
		...
		/*
			...
			어떤 특정 tag를 click했을때 원하지 않는 event가 발생하는 경우가 있다. 
			이러한 현상을 이벤트 버블링 이라고 한다.
			tag box의 가장 중간부분에 있는 tag를 click하면 안쪽부터 바깥쪽으로 
			계속해서 이벤트가 전해지는 현상
		...
		document.querySelector("table").addEventListener("click",
				function(event) {
					...
					if (tag_name === "TD") {
						...
						// tr 을 참조하겠다.
						// 클릭된 TD를 기준으로 TR tag에서 seq 값 추출
	...
	<tbody>
		...
		<c:forEach items="${BBS_LIST}" var="vo" varStatus="i">
			<tr class="bbs-tr" data-seq="${vo.b_seq}" >
				...
				<td>${vo.b_time}</td>
				<td>${vo.b_writer}</td>
				<td data-seq="${vo.b_seq}" class="bbs-subject"> ${vo.b_subject} <img src="${rootPath}/upload/${vo.b_file}" width="50px">
				</td>
				<td>${vo.b_count}</td>
			</tr>
		</c:forEach>
