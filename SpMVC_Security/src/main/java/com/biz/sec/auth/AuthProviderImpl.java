 ...
 * authentication-provider 클래스를 Customizing 수행하기
 */
@Slf4j
public class AuthProviderImpl implements AuthenticationProvider {
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		if( !bUser ) {
			 ...
			 * 
			 * spring security login 이 진행되는 도중에
			 * 어떤 문제가 발생을 하면
...
//			throw new UsernameNotFoundException(msg);
		...
		if(!userVO.isEnabled()) {
			throw new DisabledException("사용자정보를 사용할 수 없습니다");
		}
		if(!userVO.isAccountNonLocked()) {
			throw new LockedException("사용자계정이 잠겨잇습니다. 관리자에게 문의 하세요");
		}
