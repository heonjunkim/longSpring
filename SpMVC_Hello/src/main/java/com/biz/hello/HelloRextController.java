...
import com.biz.hello.service.HelloScoreService;
import com.biz.hello.service.HelloService;


/*
 *@RestController
 * spring frame work 4.x 이상에서 지원되는 새로운 controller
 * 
 */
@RestController
public class HelloRextController {
	 ...
	 * 컨테이너가 있으며
	 * 
	 * 컨테이너에 저장된 객체를 찾아서 선언된 객체에 주입하여 초기화, 사용할 수 있도록 만들어 준다.
	 * 객체를 생성하기 위해서 생성자를 만들 필요가 없어졋다.
	 */
	@Autowired
	private HelloService hService;
	
	@Autowired
	private HelloScoreService sService;
	
	@Autowired
	private HomeController hController;
	/*
	 * 표준 자바코드에서 사용하는 (기본) 생성자
	public HelloRextController() {
		hService = new HelloServiceimpl();
		sService = new HelloScoreService();
	}
	*/
	 ...
	 * 
	 * 프로젝트가 시작될때마다 @Annotation이 붙어있는 모든 클래스는 이미 객체로 생성되어
	 * Container에 저장되어 있다.
	 * 
	 ...
	 */
//	public HelloRextController(HelloService hService, HelloScoreService sService) {
...
//	}
	 ...
	 * 초기화가 되지 않아서
	 * 
	 * intList.add(100);을 실행했을때 NullPointerException이 발생한다.
	 */
