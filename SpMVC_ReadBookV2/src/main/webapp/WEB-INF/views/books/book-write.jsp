...
<script>
	// 컨트롤러에서 보내준 _csrf.headerName 과 _csrf.token 값을 JS 파일로 전달하기 위해서
		// 스크립트 변수를 선언하고
...
<%
/*
@SessionAttributes(), @ModelAttribute(), Spring form tglib를 연동한 write(입력 form) 구현

Controller class에 @SessionAttributes("bookVO")를 설정하고
각 method에 매개변수로 @ModelAttribute("bookVO") bookVO BookVO 를 선언하고
Controller class의 맴버 영역에 @ModelAttribute("bookVO") public BookVO netBookVO() { } method를 선언하고
Spring form taglib를 이용한 write form에 <form:form modelAttribute="bookVO">를 선언하여 
프로젝트를 구현하면 
id, seq등 실제 사용자에게 입력받거나, 보여줄 필요가 없는 VO의 변수들을 
<input type="hidden">으로 설정한 후 Controller로 전송하던 HTML 5 표준방식을 사용하지 않아도
VO 에 설정된 변수들을 Controller와 JSP가 서로 공유하여 사용할 수 있다.
@SessionAttributes()에 담긴 VO 객체는 서버의 메모리에 보관되며
HTTP 프로토콜의 비연결지향(상태가 없는 통신)통신상태에서도 데이터를 서로 자유롭게 공유하여 
구현할 수 있는 장점이 있다.

그럼에도 경우에 따라서 입력 form을 사용자에게 보여주었을때 최종 마지막에 입력했던 데이터들이 
