 ...
 * 사용자가 login form에 username과 password를 입력한 후 login을 시도하면
 * 1.AuthenticationProvider 인터페이스를 구현한 AuthProviderImpl 객체를 찾는다.
 * 
 * 2. authenticate() method를 호출한다.
 * 		이때 Spring security 는 authentication 객체에 로그인한 
 * 		username, password를 실어서 전달한다.
 * 
 * 3. authentication 객체에서
 * 			 principal(username), credential(password) 값을 getter 하여
 ...
 * 		user(member) table에서 select 하여 return 한다.
 * 		이때 username 에 해당하는 사용자 정보가 없으면 
 * 		UserNameNotFoundException을 발생시켜 spring security에게 사용자 정보가
 * 		없음을 통보한다.
 * 
 *  6. UserDetailsServiceImpl로 부터 return 받은 UserDetails에 담겨있는
 *  	 password와 사용자가 login한 password를 비교한다.
 *  	 만약 Password가 다르면 
 *  		BadCredentialsException을 강제로 발생시켜 spring security에게 통보한다.	
 * 		  
 *  7. username과 password 가 확인 되었다! 이제 사용자 정보에 enable 값을 검사하여
...
public class AuthProviderImpl implements AuthenticationProvider {
	...
	/*
	 * 사용자가 로그인을 수행했을때 username과 , password를 주입받아서 정상적인 사용자인가를 검사하는 method
	 */
	@Override
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		/*
		 * if(!password.equals(userVO.getPassword())) { // 비밀번호가 일치하지 않으면 throw new
		 * BadCredentialsException("비밀번호를 잘못 입력하셨습니다."); }
		 */

		// PasswordEncoder로 암호화된 비번 비교
		/*
		 * 사용자가 입력한 password 평문 문자열을 내부에서 암호화 하여 DB에 저장되어 있는 암호화된
		 * 비번(userVO.getPassword())을 비교하여 일치하는지 검사한다.
		 */
