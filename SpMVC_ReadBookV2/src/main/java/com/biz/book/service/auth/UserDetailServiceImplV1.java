...
public class UserDetailServiceImplV1 implements UserDetailsService {
	 ...
	 * @Autowired로 주입받은 객체에 메모리 누수현상이 발생을 한다
	 * 
	 ...
	 * 3. 주입받을 객체의 개수가 늘거나 줄면 생성자를 또다시 변경 해야 하는
	 *  	번거로움이 있다. 
	 * 
	...
	*/
	 ...
	 * 기능의 수행을 제한하는 설정을 하고
	 * 사용자의 여러 세부 정보를 VO객체에 담아 주는 역할 수행
	 */
	@Override
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		// DB로 부터 가져와서 GrantedAuthority로 변환한 ROLE 정보 List를 UserDetails 정보에 저장
	...
	private Collection<GrantedAuthority> getAuthorities(String username) {
		...
		for(AuthorityVO vo : authList) {
			...
			// 문자열로된 ROLE 정보를 GrantedAuthority 타입으로 변환하여 
			// 리스트로 생성하기
