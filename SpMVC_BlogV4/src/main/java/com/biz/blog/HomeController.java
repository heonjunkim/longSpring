...
public class HomeController {
	...
	/*
	 * form에서 input box에 입력한 내용은 http protocol에 의해 네트워크를 건너서 Server로 전송이 되고 그 데이터는
	 * Controller의 함수에서 받을수 있다.
	 * 
	 * 함수에는 input box의 name으로 설정된 것과 같은 이름의 문자열 변수를 매개변수로 설정만 해두면 나머지는
	 * SpringFramework가 자동으로 값을 수신해서 변수에 담아준다.
	 */
