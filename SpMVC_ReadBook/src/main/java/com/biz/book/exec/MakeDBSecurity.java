 ...
 * DB 접속용 username과 password를 암호화 하여 
 * 문자열을 추출
 */
...
public class MakeDBSecurity {
	public static void main(String[] args) {
		...
		System.out.printf("BIZ.BET 환경변수 : %s\n",saltPassword);
		...
		// salt : 암호화를 할때 사용할 key문자열 탈취하기가 매우어려워진다 
		/*
		 ...
		 * 
		 * 문자열을 바로 사용하지 않고 
		 * 시스템(컴퓨터 운영체제)에 환경변수를 저장해 두고
		 * 환경변수를 가져다 salt 비밀번호로 사용을 한다.
		 * 
		 * BIZ.COM 이라는 문자열이 노출되어도
		 * 네트워크를 통해서 salt 암호를 추출하기가 매우 어려워진다.
		 * 
		 * 따라서 암호화하는 효과를 좀더 배가 시킬수 있다.
		 */
		...
		scan.close();
		System.out.println("DB연결 속성 파일 생성 완료!!");
	
		// Exception in thread "main" java.lang.IllegalArgumentException: Password cannot be set empty전역환경변수가 아닐때
/*		for(int i =0; i <10; i++) {
			// 평문을 암호화하여 암호문 추출 : encryption
			String encText = pbe.encrypt(planText);		
			System.out.printf("P :%s, E :%s\t",planText,encText);
			
			// 암호문을 복호화하여 평문 추출 : descryption
