...
public class OrderController {
	...
	/*
	 * 주문서 전체리스트를 DB로부터 가져와서 보여주는 Controller
	...
	// a href = "input"로 설정된 링크를 클릭햇을때 응답할 method
	...
	public String input(
		@ModelAttribute	 OrderVO orderVO, Model model) {
	...
	/*
	 * Browser에서 Order request를 실행할때(요청을할때)
	...
	public String getOrder(String seq, String name, Model model) {
		...
		/*
		 * Dispacther야 /WEB-INF/views/order/view.jsp 파일을 읽어서
		 * 클라이언트(요청하는곳)으로 응답을 하여라
	...
	public String insert(
		...
		System.out.println("날짜:" + po_date);
		System.out.println("고객번호:" + o_cnum);
		System.out.println("상품번호:" + o_pcode);
		OrderVO orderVO = new OrderVO();
		

		
		orderVO.setO_num(o_num);
		orderVO.setPo_date(po_date);
